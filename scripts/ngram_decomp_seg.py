...
def get_ranking_of_predictors(words, index, spans,trie,wild_card_trie,total_token_count):
    ...
    to_sort = zip(results.values(), map(get_len,results.keys()), map(convert_span,results.keys()))
    to_sort.sort()
    return to_sort
...
def segment_corpus_by_decomposition(ngram_trie,wild_card_ngram_trie,total_token_count,options,break_trie={}):
    '''
    ...
    for sentence in read_sentence_from_corpus(f):
        ...
        for span in chunked_spans:
            ...
            taken_indicies.update(range(span[0],span[1]))
        for i in range(len(sentence)):
...
if __name__ == "__main__":
    ...
    parser.add_option("-s","--segment",dest="segment",help="output a corpus segmentation, multiword units are joined by underscore")
    ...
    parser.add_option("-l","--lines",type="int",dest="lines",help="Limit the number of lines (sentences) of the corpus processed, useful for testing")
    options,arguments = parser.parse_args()
    if not options.corpus or not (options.segment or options.output):
        parser.print_help()
    else:
        if not options.vocab: # do end-to-end segmentation 
            ...
            counts_trie = segment_corpus_by_decomposition(reg_trie,wild_trie,total_token_count,options)
            ...
            if options.segment:
                segment_corpus_by_decomposition(reg_trie,wild_trie,total_token_count,options,break_trie=break_trie)
        else: # just load vocabulary and use it to segment
            if not options.segment:
                parser.print_help()
            else:
                reg_trie,wild_trie,index_dict,total_token_count = load_multiword_vocab(options)
                counts_trie = segment_corpus_by_decomposition(reg_trie,wild_trie,total_token_count,options)
                if options.output:
                    fout = open(options.output,"w")
                    output_trie(counts_trie,fout,[],rev_index_dict)
