storyFinder<-function(K,date,hour,minute){
	...
	E_g<-read.csv(paste("/users/storyfinder/data/entropy_timeseries.csv",sep=""),sep=",",dec=".",quote="\"",colClasses=c("numeric","numeric","numeric","numeric","numeric","numeric","numeric","numeric","numeric","numeric","numeric","character","character","character"),header=F)
	...
	V_g<-read.csv(paste("/users/storyfinder/data/valence_timeseries.csv",sep=""),sep=",",dec=".",quote="\"",colClasses=c("numeric","numeric","numeric","numeric","numeric","numeric","numeric","numeric","numeric","numeric","numeric","character","character","character"),header=F)
...
for (k in 0:K){
	...
	if (k==0){	
	   	...
	   	E_temp_shifts=E_f$shiftCompG*E_f$pCompG-E_f$shiftRefG*E_f$pRefG
		V_temp_shifts=V_f$shiftCompG*V_f$pCompG-V_f$shiftRefG*V_f$pRefG

	   	E_shifts=sort(abs(E_temp_shifts),decreasing=T,index.return=T)
		V_shifts=sort(abs(V_temp_shifts),decreasing=T,index.return=T)
		
		E_indices=E_shifts$ix
		V_indices=V_shifts$ix

		E_diff=E_f$shiftCompG[E_indices]
		V_diff=V_f$shiftCompG[V_indices]
		
		E_shifts=E_temp_shifts[E_indices]
		V_shifts=V_temp_shifts[V_indices]
	}
	else{
		E_order_indices=which(E_f$order==k)
		V_order_indices=which(V_f$order==k)

		E_temp_shifts=E_f$shiftCompL[E_order_indices]*E_f$pCompL[E_order_indices]-E_f$shiftRefL[E_order_indices]*E_f$pRefL[E_order_indices]
		V_temp_shifts=V_f$shiftCompL[V_order_indices]*V_f$pCompL[V_order_indices]-V_f$shiftRefL[V_order_indices]*V_f$pRefL[V_order_indices]

		E_shifts=sort(abs(E_temp_shifts),decreasing=T,index.return=T)
                V_shifts=sort(abs(V_temp_shifts),decreasing=T,index.return=T)

                E_indices=E_order_indices[E_shifts$ix]
                V_indices=V_order_indices[V_shifts$ix]

		E_diff=E_f$shiftCompL[E_indices]
                V_diff=V_f$shiftCompL[V_indices]

                E_shifts=E_temp_shifts[E_shifts$ix]
                V_shifts=V_temp_shifts[V_shifts$ix]		
	}
	...
	for (i in 20:1){		
		...
		if (V_diff[i]>=0){
                        valList=rbind(valList,c(V_shifts[i],"lightgoldenrod",V_f$phrase[V_indices[i]])) #(shift,color,phrase)
                } else {
                        valList=rbind(valList,c(V_shifts[i],"steelblue",V_f$phrase[V_indices[i]])) #(shift,color,phrase)
                }		
	}
	...
	par(las=2,mfrow=c(2,2),fig=c(0.01,0.54,0,0.62))
	barplot(as.numeric(entList[,1]),col=entList[,2],horiz=T,xlim=c(-max(abs(as.numeric(entList[,1]))),max(abs(as.numeric(entList[,1]))))*1.1,xaxt="n",ylim=c(0,25.5))#,main=quote(S["comp"]~~~~~~S["ref"]))

	if (length(E_entRight)){
		text((1:length(E_entRight))*0,E_HeightRight,E_entRight,pos=4,cex=0.5)
	}
	if (length(E_entLeft)){		
		text((1:length(E_entLeft))*0,E_HeightLeft,E_entLeft,pos=2,cex=0.5)
	}
	...
	par(las=2,mfrow=c(2,2),fig=c(0.0725,0.51,0.05,0.54))
	box(which="figure",lty=1,lwd=1)
	
	# do the valence shift
	par(las=2,fig=c(0.46,0.99,0,0.62),new=T)
	...
	if (length(V_valRight)){
		text((1:length(V_valRight))*0,V_HeightRight,V_valRight,pos=4,cex=0.5)
	}
	if (length(V_valLeft)){
		text((1:length(V_valLeft))*0,V_HeightLeft,V_valLeft,pos=2,cex=0.5)
	}
	...
	par(las=2,fig=c(0.525,0.9625,0.05,0.54),new=T)
	box(which="figure",lty=1,lwd=1)
	...
	plot((1:length(E_timeseries))[E_timeseries>0],E_timeseries[E_timeseries>0],pch=20,xlab="",ylab="(bits)",main="Timeseries'",type="l",cex=0.5,xaxt='n',col="black",cex.axis=0.6)
	...
	plot((1:length(V_timeseries))[V_timeseries>0],V_timeseries[V_timeseries>0],pch=20,xlab="",ylab="(happs)",main="",type="l",cex=0.5,xaxt='n',col="black",cex.axis=0.6)
