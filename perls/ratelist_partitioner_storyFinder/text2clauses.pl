...
while ($line=<$corpus>){
    ...
    if (!(($line =~ m/more for aries/gi) || ($line =~ m/more for taurus/gi) || ($line =~ m/more for gemini/gi) || ($line =~ m/more for cancer/gi) || ($line =~ m/more for leo/gi) || ($line =~ m/more for virgo/gi) || ($line =~ m/more for libra/gi) || ($line =~ m/more for scorpio/gi) || ($line =~ m/more for sagittarius/gi) || ($line =~ m/more for capricorn/gi) || ($line =~ m/more for aquarius/gi) || ($line =~ m/more for pisces/gi) || ($line =~ m/4sq\.com/gi) || ($line =~ m/extra watering cans after harvesting/gi) || ($line =~ m/\@questionnnierr/gi))){
